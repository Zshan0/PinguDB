...
void executeCROSS() {
  ...
  // If both tables are the same i.e. CROSS a a, then names are indexed as a1
  // and a2
  ...
  for (int columnCounter = 0; columnCounter < table1.columnCount;
       columnCounter++) {
  ...
  for (int columnCounter = 0; columnCounter < table2.columnCount;
       columnCounter++) {
  ...
  Table *resultantTable =
      new Table(parsedQuery.crossResultRelationName, columns);
