...
bool syntacticParsePROJECTION() {
  ...
  parsedQuery.projectionRelationName =
      tokenizedQuery[tokenizedQuery.size() - 1];
...
void executePROJECTION() {
  ...
  for (int columnCounter = 0;
       columnCounter < parsedQuery.projectionColumnList.size();
       columnCounter++) {
    columnIndices.emplace_back(
        table.getColumnIndex(parsedQuery.projectionColumnList[columnCounter]));
  }
  ...
  while (!row.empty()) {
    ...
    for (int columnCounter = 0; columnCounter < columnIndices.size();
         columnCounter++) {
