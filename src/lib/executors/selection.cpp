...
/**
 * @brief
 * SYNTAX: R <- SELECT column_name bin_op [column_name | int_literal] FROM
 * relation_name
 */
...
void executeSELECTION() {
  ...
  int firstColumnIndex =
      table.getColumnIndex(parsedQuery.selectionFirstColumnName);
  int secondColumnIndex;
  if (parsedQuery.selectType == COLUMN)
    secondColumnIndex =
        table.getColumnIndex(parsedQuery.selectionSecondColumnName);
  while (!row.empty()) {
